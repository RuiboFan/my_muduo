...
void TcpConnection::sendInLoop(const void* data, size_t len) {
    ...
    // 之前调用过该connnection的shutdown， 不能再进行发数据了
    ...
    // 说明当前这一次write，并没有把数据全部发送出去，剩余的数需要保存到缓冲区中，然后给channel
    ...
    if (!faultError && remaining > 0) {
        ...
        if (!channel_->isWriting()) {
            channel_->enableWriting();   // 这里一定要注册channel的写回调，负责poller不会给channel通知epollout
        }
    }
}
